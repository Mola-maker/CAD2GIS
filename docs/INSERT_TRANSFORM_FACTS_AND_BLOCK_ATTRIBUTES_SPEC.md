...
CAD2GIS 的 PTECH / BOITE / SITE 等设备在图中的几何是 INSERT（块引用）点；**这些设备在 DWG 里的真实标识文字，往往不在 Model 空间的独立 TEXT 上，而在 INSERT 引用的块定义（BLOCK DEFINITION）里**。例如：
...
2. `semantics.classify_entities` 只能看到 Model 空间文字，见不到块内文字；PTECH 的 `CODE` 全部退化为 `PTECH-CAD-<handle>`（provenance=`DWG_DERIVED:stable-handle-id`），不是原 DWG 文字。
3. 即使 Model 空间有标签，标注归属仍退化为“文本插入点 ↔ 目标点”的最近邻欧氏距离，未利用块/owner 关系。

### 1.1 已核实的证据（当前 HEAD `a5ebe97` 的 run 产物）
...
- `src/cad2gis/reader/libredwg.py:1212-1252`：INSERT 分支（已读 `ins_pt.x/y`、`scale.x/y/z`、`rotation`、block name、owner ATTRIB）
- `src/cad2gis/reader/libredwg.py:1299-1342`：`raw_properties` 把 `insertion_point` / `block_base_point` / `normal` / `extrusion` 全部标 `not_applicable`，`transform_facts={}`
- `src/cad2gis/cad2gis_v3/ports.py:150-289`：`_transform_facts()` 的 fail-closed 契约
- `src/cad2gis/cad2gis_v3/ports.py:528-542`：`resolve_insert_affine()` 公共边界
- `src/cad2gis/cad2gis_v3/plan_domain.py:394-490`：INSERT 展开调用 `resolve_insert_affine()`，缺 facts 则 abstain（当前非 blocking warning，因此 run 继续但块内文字丢失）
...
4. **真实标签优先于生成 handle。**
   PTECH `CODE` 的 provenance 在可解析标签处不再是 `DWG_DERIVED:stable-handle-id`，而是 `DWG_DERIVED:<family>|RULE:...` 或 `DWG_DIRECT:...`；无可解析标签时仍保留 fail-closed 的生成 handle。
5. **确定性回归不破坏。**
...
| `block_attributes` | `owner_attribs` 已按 owner handle 预索引（`libredwg.py:1467-1487`、`1237-1252`） | 需验证嵌套块、tag 提取和 value 字段；四个 DWG 中目前仍有 `libredwg_block_attributes_unread` 记录 |
...
- DSH-011：真实 DWG 标签允许覆盖 `PTECH-CAD-*` 生成 handle；生成 handle 只是 fail-closed 占位，不算语义标签。
...
- Reader：`src/cad2gis/reader/libredwg.py`
  - INSERT 分支：1212-1252
  - raw_properties：1299-1342
  - owner ATTRIB 预索引：1467-1487
  - block header name：767-791
- Transform 契约：`src/cad2gis/cad2gis_v3/ports.py`
  - `_transform_facts`：150-289
...
- 语义标注：`src/cad2gis/cad2gis_v3/semantics.py`
  - 标注目标资格（本 session 已修）：`_annotation_target_eligible`
  - `_assign_family_annotations`：311-401
- 诊断工具：`tools/diagnostics/compare_runs.py`（本 session 新增）
