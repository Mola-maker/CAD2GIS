...
审计日期：2026-09-04。原始审计基于 `main` / `39b5625`，以下候选表、规模和行号记录清理前状态；A1/A2/A3 随后根据用户要求执行，结果见下节。`src/cad2gis/cli.py` 已有的两处未提交注释修改保留。
...
B/C 候选未执行，包括旧 AutoCAD 公开转换 API、semantic anchor、records adapter、Web 去重以及失联策略守卫的修复。它们涉及接口/用途选择或行为变更，应按原候选说明分别处理。
