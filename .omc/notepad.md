...
### 2026-08-19 图纸语义澄清（用户确认，长期有效）

- **APD = As Plan Drawing**：建筑工程领域“按计划图纸”，是 as-planned
  设计态；不是 as-built 竣工实测，也不是接入点设备。转换与精度声明必须
  区分“计划图几何”与“地面实测”。
- **SF = Subfeeder**：副馈线 / 分支配电线。文件名后缀 `- SF` 表示该图是
  网络的 subfeeder 部分（如 `lamteh_sf`）。
- **语料切分**：`raw/` 原 4 张为开发/基线集；新加入 6 张为验证集。验证集
  每张必须各自 source-bound 处理，禁止复用基线规则、计数或阈值。
