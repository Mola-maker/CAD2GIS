...
| 5 | 法标 FTTH 验证规则库 | `experiment/evaluation_standards/` 全目录（VERIFICATION_RULE.csv、BOITE.csv、CABLE.csv、PTECH.csv、SITE.csv、ZNRO.csv、ZPM.csv、INFRASTRUCTURE.csv，共 8 份） | 直接用 | 业务字段级验证规则（必填/取值域/计数期望）；可接入 newmodel 验证矩阵的业务维度 |
...
| # | 算法 | 来源路径（main 分支） | 说明 |
| --- | --- | --- | --- |
| 8 | 匈牙利一对一标注分配 | `experiment/py_scripts/converter.py` 的 `_minimum_cost_assignment`（约 773-831 行） | 标注→几何的全局最优二分匹配（而非最近邻贪心），避免跨街区误关联；与 newmodel「同类候选二分图最大匹配」同族，可对照实现 |
...
| 10 | 跨度注记持久化 | `experiment/py_scripts/converter.py` 的 `_write_span_annotations`（约 3093-3303 行） | 170 条「xx.x m」标注落库为 span_annotations 表并带真 FID 外键；与 newmodel `CABLE_SEGMENT.dimension_entity_key` 机制互为印证，可作为标注-段关联的审计字段设计参考 |
...
计数差异（如 CABLE 203 vs 6）不是一方错误，而是**交付语义
不同**：main 聚合业务网络，newmodel 保真源几何并规范化派生层。审计时请按
各自口径解读，不要横向比大小。

> **BOITE 口径注**（2026-07-20 项目所有者确认）：main 交付的 43 个为**带标签**
> BOITE（值为 16）；另有 **2 个不带标签**的 BOITE 分别位于两个 SITE 位置
> （值为 48、72）——这是程序的局限性，需人工干预补录；物理总数为 45（即
> 修复前旧基线口径）。newmodel 侧的 43 建议按同样口径核对：若同为 43 带
> 标签，则两路线在该层一致，剩余 2 个不入自动交付属预期行为。
