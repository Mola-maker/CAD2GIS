...
def _write_staged(path, entities, features, relations, unresolved, diagnostics, source_srs):
    ...
    for entity in entities:
        if entity.entity_key in feature_sources:
            disposition = "mapped"
        elif entity.entity_key in annotation_sources or entity.dwg_type in {"TEXT", "MTEXT", "DIMENSION"}:
    ...
    annotation_table = _table(dataset, "annotation_assignment_candidates", [
        ("annotation_key", ogr.OFTString), ("text", ogr.OFTString),
        ("target_class", ogr.OFTString), ("target_key", ogr.OFTString),
        ("target_handle", ogr.OFTString), ("distance_native_m", ogr.OFTReal),
        ("selected", ogr.OFTInteger), ("status", ogr.OFTString),
        ("rule_id", ogr.OFTString),
    ])
    ...
    for feature in features:
        ...
        for field_name in sorted(mandatory | set(feature.attributes)):
            ...
            _set(row, {
                "feature_key": feature.feature_key, "feature_class": feature.feature_class,
                "field_name": field_name, "field_value": "" if value is None else str(value),
    ...
    unresolved_table = _table(dataset, "unresolved_items", [
        ("kind", ogr.OFTString), ("status", ogr.OFTString), ("detail", ogr.OFTString),
    ])
    for item in unresolved:
        row = ogr.Feature(unresolved_table.GetLayerDefn())
        _set(row, {"kind": str(item.get("kind", "")), "status": str(item.get("status", "")), "detail": json.dumps(item, ensure_ascii=False, sort_keys=True)})
...
def write_evidence(path, entities, features, relations, unresolved, diagnostics, source_srs):
    ...
    try:
        _write_staged(staged, entities, features, relations, unresolved, diagnostics, source_srs)
