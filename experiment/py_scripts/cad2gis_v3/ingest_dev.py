"""Dev-only ingestion wrapper that uses the LibreDWG dev-reader.

This module mirrors ``cad2gis_v3.ingest`` but swaps the reader backend to
``libredwg_dev_reader.extract_dwg_records``.  It is intentionally isolated
from the canonical ``ingest.py``; canonical production ingestion remains
unchanged.

The synthetic-metadata gate requires ``CAD2GIS_DEV_READER=1`` whenever the
reader could not read CGEOCS/INSUNITS from the DWG header and had to
synthesise the evidence record.
"""
...
from libredwg_dev_reader import extract_dwg_records

from .config import SourceProfile
from .model import SourceEntity

_SYNTHETIC_MARKER = "__CAD2GIS_SYNTHETIC_METADATA_EVIDENCE_7f3a9c__"
...
def ingest(source: str | Path, profile: SourceProfile) -> tuple[list[SourceEntity], dict]:
    ...
    if (
        ...
        raise RuntimeError(
            "LibreDWG dev-reader inventory is incomplete; compatibility-mode "
    ...
    # Fail-closed gate: synthetic metadata evidence requires explicit dev opt-in.
    metadata_is_synthetic = _SYNTHETIC_MARKER in metadata
    if metadata_is_synthetic and os.environ.get("CAD2GIS_DEV_READER") != "1":
        raise RuntimeError(
            "synthetic metadata evidence requires CAD2GIS_DEV_READER=1"
        )
    # Contradiction guard: reader claims real evidence but marker is present.
    if reader_protocol.get("metadata_evidence") == "reader" and metadata_is_synthetic:
        raise RuntimeError(
            "reader metadata_evidence is 'reader' but metadata contains synthetic marker"
        )
