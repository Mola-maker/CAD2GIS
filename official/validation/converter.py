...
try:
    ...
    def validate_domain_value(fc_name, field_name, value):
        """Stub when domain_vocab.py is unavailable."""
        return True, []
...
# Default: EPSG:32629 (WGS84 / UTM zone 29N — Morocco/West Africa)
...
def read_dwg(dwg_path):
    ...
    point_types = {DWG_TYPE_TEXT, DWG_TYPE_MTEXT, DWG_TYPE_INSERT, DWG_TYPE_POINT}
    ...
    for i in range(data.num_objects):
        ...
        raw_features.append({
            ...
            "block_name": _get_block_name(entity) if dwg_type == DWG_TYPE_INSERT else "",
        })
    ...
    for i, rf in enumerate(raw_features):
        ...
        if pts and len(pts) >= 2:
            ...
            # Flag but don't discard — just mark for review
            rf["suspicious_span"] = span_deg > 1.0
        else:
            rf["suspicious_span"] = False

        # Classification: TEXT entities → annotation (not geometry)
        if rf["dwg_type"] in (DWG_TYPE_TEXT, DWG_TYPE_MTEXT) and not rf["is_insert_node"]:
...
def main():
    ...
    parser.add_argument("--source-crs", default="EPSG:32629",
                       help="Source DWG CRS (default: EPSG:32629 = WGS84/UTM zone 29N). "
                            "Use 'EPSG:4326' for identity/no reprojection.")
    ...
    for dwg_path in args.input:
        ...
        if file_outliers:
            warnings_log.append(
                ...
                f"outside Morocco bounds (not discarded, flagged only)"
            )
        if file_misc:
            warnings_log.append(
