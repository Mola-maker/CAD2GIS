...
> The target domain is **Francophone FTTH telecom deployment** (8 GIS feature classes: BOITE,
...
DOMAIN: Francophone FTTH telecom deployment. Source CRS: EPSG:4326 WGS84 (authoritative).
...
WHAT YOU DO:
  ...
  3. Check Morocco deployment bounds: lat ∈ [21, 36], lon ∈ [-17, -2].
     Entities outside → GEOGRAPHIC_OUTLIER_DETECTED (warning, NOT halt).
     Rationale: remote relay towers may be outside urban Morocco bbox.
  4. Multi-file consistency: if File 0 bbox spans [30-34N, -6 to -3E] and
...
Input:
  ...
  morocco_bounds: {"lat": [21, 36], "lon": [-17, -2]}
...
For each tile_id in manifest:
  ...
  4. Count outliers outside Morocco bounds.
...
COORDINATE TRANSFORM CACHE [I4 — First-Hit Coordinate Cache]:
  ...
  Cache miss: validate against Morocco bounds → populate cache.
...
Processing:
  ...
  3. For each entity in tile:
     ...
     b. For POINT-type entities: check coord_cache before Morocco bounds validation.
...
OPERATION 1: NODE SNAPPING [critical for FTTH network integrity]
  ...
  within SNAP_TOL_DEG = 0.0001° (~11m at Morocco latitude).
...
OPERATION 5: SLIVER ELIMINATION
  Polygons with area < SLIVER_AREA_DEG2 = 1e-8° (~0.12 m² at Morocco lat) AND
...
Q3 — COORDINATE PRECISION                  [Mode B | TS Part VIII Rule Q3 = B2]
  ...
  Target: ≤ 1×10⁻⁵ degrees (~1.1m at Morocco latitude)
...
*Document: GeoFormer-FiberHome P2 Agent Prompts v1.1*
*Revised from v1.0 following code agent discriminating probe (ZPM.REF_SRO authority fix,*
*Agent 8 dual-scope declaration, FK5b added, containment checks GC1-7 added,*
*Appendix B schema mapping completed for all 8 FCs with field name normalisation map)*
*Feature Classes: BOITE, CABLE, PTECH, IMB, INFRASTRUCTURE, SITE, ZNRO, ZPM*
*Output CRS: EPSG:4326 | QGIS Display CRS: EPSG:3857 (OSM) | OTF reprojection*
