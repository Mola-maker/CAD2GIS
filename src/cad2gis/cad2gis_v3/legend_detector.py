...
def filter_legend_entities(
    entities: Iterable[Any],
    *,
    gap_min: float = DEFAULT_GAP_MIN,
    gap_k: float = DEFAULT_GAP_K,
    min_confidence: float = DEFAULT_MIN_CONFIDENCE,
    confirmed_regions: Iterable[dict[str, Any]] | None = None,
    auto_exclude: bool = False,
) -> dict[str, Any]:
    """Detect legend clusters among plan-domain SourceEntity objects.

    Converts SourceEntity fields to the ``detect_legend_clusters`` feature-
    dict contract, then builds a ``legend_flagged_keys`` frozenset and
    diagnostics.  This adapter is a pure function — it never mutates the
    input entities.

    Args:
        entities: Iterable of objects with ``entity_key``, ``centroid``,
            ``text``, and ``layer`` attributes (typically ``SourceEntity``
            from ``plan_domain``).
        gap_min, gap_k, min_confidence: Passed through to
            ``detect_legend_clusters``.
        confirmed_regions: Optional iterable of dicts with ``bbox`` and/or
            ``member_ids`` keys.  Clusters matching a confirmed region are
            flagged with ``confirmed=True`` and may trigger auto-exclusion
            when ``auto_exclude`` is enabled.
        auto_exclude: When ``True``, entities belonging to confirmed clusters
            are removed from the returned entity list.  When ``False`` (the
            default), all entities pass through and flagged keys are only
            reported — the caller decides how to act.

    Returns:
        ``{"entities": list, "legend_flagged_keys": frozenset[str],
           "diagnostics": dict}``
    """
    entity_list = list(entities)
    diagnostics: dict[str, Any] = {
        "schema_version": "cad2gis-legend-filter-v1",
        "status": "NO_LEGEND_DETECTED",
        "feature_count": len(entity_list),
        "flagged_count": 0,
        "body_bbox": None,
        "clusters": [],
        "auto_exclude": auto_exclude,
    }
    if len(entity_list) < DEFAULT_MIN_FEATURES:
        return {
            "entities": entity_list,
            "legend_flagged_keys": frozenset(),
            "diagnostics": diagnostics,
        }

    features = []
    for entity in entity_list:
        ctr = getattr(entity, "centroid", None)
        if ctr is None:
            continue
        features.append({
            "id": getattr(entity, "entity_key", id(entity)),
            "centroid": (float(ctr[0]), float(ctr[1])),
    ...
    body_bbox = result.get("body_bbox")
    diagnostics["body_bbox"] = body_bbox
    ...
    # Build legend_flagged_keys from cluster member_ids.
    flagged: set[str] = set()
    for cluster in clusters:
        member_ids = cluster.get("member_ids", ())
        flagged.update(str(mid) for mid in member_ids)

    # Apply confirmed region overrides when provided.
    ...
    if confirmed_regions:
        for region in confirmed_regions:
            ...
            region_members = region.get("member_ids", ())
            if region_bbox and len(region_bbox) == 4:
                # Flag all entities whose centroid falls inside the confirmed bbox.
                mx, my, Mx, My = (float(v) for v in region_bbox)
                for entity in entity_list:
                    ctr = getattr(entity, "centroid", None)
                    if ctr is None:
                        continue
                    if mx <= float(ctr[0]) <= Mx and my <= float(ctr[1]) <= My:
                        key = str(getattr(entity, "entity_key", ""))
                        if key:
                            confirmed_keys.add(key)
            for mid in region_members:
                confirmed_keys.add(str(mid))

    if auto_exclude and confirmed_keys:
        # Remove confirmed entity keys from the output.
        all_excluded = flagged | confirmed_keys
        diagnostics["excluded_count"] = len(all_excluded)
        diagnostics["excluded_keys"] = sorted(all_excluded)
        diagnostics["status"] = "FILTERED"
        return {
            "entities": [
                entity for entity in entity_list
                if str(getattr(entity, "entity_key", "")) not in all_excluded
            ],
            "legend_flagged_keys": frozenset(flagged | confirmed_keys),
            "diagnostics": diagnostics,
        }

    diagnostics["flagged_count"] = len(flagged)
    diagnostics["confirmed_count"] = len(confirmed_keys)
    diagnostics["status"] = (
        "CLUSTERS_DETECTED" if flagged else "NO_LEGEND_DETECTED"
    )
    return {
        "entities": entity_list,
        "legend_flagged_keys": frozenset(flagged | confirmed_keys),
