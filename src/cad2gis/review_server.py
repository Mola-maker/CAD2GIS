...
def _roadmatch_summary(run_path: Path) -> dict[str, Any]:
    """Read the optional road-match locator report for a run.

    The report is generated by the road-match tooling and never changes
    delivery geometry; the web review surface only presents the machine
    candidates for human confirmation.
    """
    report_path = run_path / "road_match_result.json"
    if not report_path.is_file():
        return {"available": False}
    try:
        payload = _json_object(report_path)
    except (OSError, json.JSONDecodeError, ReviewServerError):
        return {"available": False, "error": "road_match_result.json is invalid"}
    overlay_path = run_path / "reasoning" / "roadmatch_overlay.png"
    payload = dict(payload)
    payload["available"] = True
    payload["overlay"] = (
        "/api/roadmatch/overlay.png" if overlay_path.is_file() else None
    )
    return payload
    ...
    *,
    ...
    async def run_summary():
        return {
            ...
            "road_match": _roadmatch_summary(run_path),
            "qgis_server": {
                "url": qgis_server_url,
    ...
    @app.get("/api/roadmatch")
    async def roadmatch_summary():
        return _roadmatch_summary(run_path)

    @app.get("/api/roadmatch/overlay.png")
    async def roadmatch_overlay():
        overlay_path = (run_path / "reasoning" / "roadmatch_overlay.png").resolve()
        if not overlay_path.is_file():
            raise HTTPException(404, "Road-match overlay is not available")
