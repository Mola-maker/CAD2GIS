...
def _unsupported_count(sample: Mapping[str, Any], dimension: str) -> int | None:
    names = {
        ...
        # A run-level ``unresolved_count`` can describe topology, port, or
        # annotation candidates and must not be attributed to semantics.  A
        # semantic coverage record has its own explicit unknown/unmatched
        # counts and fail-closed status.
        "semantics": ("unknown_count", "unmatched_count", "unclassified_count", "unknown", "unmatched", "unresolved"),
