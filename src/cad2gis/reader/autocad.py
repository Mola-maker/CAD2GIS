...
def build_items_from_records(records, source_name, reproject_point, assign_fc, classify_block, extract_attributes):
    """Compatibility entry point for the historical records-to-GIS converter."""
    from cad2gis.legacy.autocad_conversion import build_items_from_records as convert
...
def read_dwg_with_autocad(
    ...
    *,
    ...
    """Compatibility entry point for the historical direct-DWG GIS converter."""
    from cad2gis.legacy.autocad_conversion import read_dwg_with_autocad as convert
...
_LEGACY_CONVERSION_HELPERS = frozenset({
    "_point_wkt",
    "_line_wkt",
    "_polygon_wkt",
    "_evidence_item",
...
def __getattr__(name):
    """Resolve historical conversion helpers only when explicitly requested."""
    if name in _LEGACY_CONVERSION_HELPERS:
        from cad2gis.legacy import autocad_conversion

        return getattr(autocad_conversion, name)
