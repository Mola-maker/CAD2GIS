...
Uses hardcoded domain values — no external file dependencies.
"""
...
# Module-level cache — uses hardcoded values only (no external CSV dependency)
_domains_cache = None
...
def _get_domains():
    ...
    if _domains_cache is None:
        _domains_cache = dict(HARDCODED_FALLBACK)
        _domains_cache.setdefault("MODE_POSE_BOITE", list(MODE_POSE_BOITE_VALUES))
