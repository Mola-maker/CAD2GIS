...
# ── Project config override (loaded from --config JSON) ────────────────────
...
# ── Shared DWG library (L1-L2) ──────────────────────────────────────────
from . import cad_common
from .cad_common import (
    init_crs,
    _reproject_point, _to_wgs84, _valid_coord, _in_region_bounds,
    _meters_to_units, _line_length_m,
    _init_libredwg, _entity_utf8_text, _cstr, _layer_name,
    _parse_dwg_color, _resolve_effective_color, _lwpoline_points,
    _adaptive_chord_tolerance, _haversine, _haversine_length,
    _centroid, _sha256_file, _safe_layer_name,
    _wkt_point, _wkt_linestring, _wkt_polygon_exterior,
    _circle_points, _arc_points, _extract_wkt, _extract_dimension,
    _cluster_points,
    aci_to_rgb,
    ...
    DWG_TYPE_ATTRIB, DWG_SUPERTYPE_ENTITY,
    DIMENSION_TYPE_UNION, CONTROL_TYPES,
    DEFAULT_FRAGMENT_CLUSTER_TOL_M, DEFAULT_ANNOTATION_LINK_TOL_M,
    DEFAULT_BOITE_FUSION_TOL_M, METERS_PER_DEGREE,
    REGION_BOUNDS_WGS84, WEBMERC_MAX_X, WEBMERC_MAX_Y,
)
...
def read_dwg(dwg_path, fragment_cluster_tol_m=DEFAULT_FRAGMENT_CLUSTER_TOL_M,
    ...
    # Portable SWIG bindings search
    _swig_paths = [_env for _env in [os.environ.get("LIBREDWG_PYTHON", "")] if _env]
    try:
        import site; _swig_paths.append(site.getusersitepackages())
    except Exception: pass
    try:
        import sysconfig
        for _k in ("purelib", "platlib"):
            _p = sysconfig.get_path(_k)
            if _p and _p not in _swig_paths: _swig_paths.append(_p)
    except Exception: pass

    _imported = False
    for _sp in _swig_paths:
        if _sp and _sp not in sys.path: sys.path.insert(0, _sp)
        try:
    ...
    if not _imported:
        sys.exit(
            ...
            "Searched: " + ", ".join(p for p in _swig_paths if p) + "\n"
            "Set LIBREDWG_PYTHON=/path/to/LibreDWG/package/parent or install."
        )
    ...
    cad_common.DWG_TYPE_CIRCLE = L_CIRCLE
    cad_common.DWG_TYPE_ARC = L_ARC
    cad_common.DWG_TYPE_TEXT = L_TEXT
    cad_common.DWG_TYPE_MTEXT = L_MTEXT
    cad_common.DWG_TYPE_INSERT = L_INSERT
    cad_common.DWG_TYPE_POINT = L_POINT
    cad_common.DWG_TYPE_ATTRIB = L_ATTRIB
    ...
    for i, rf in enumerate(raw_features):
        ...
        if cad_common._CRS_TRANSFORM is not None:
            try:
                geom = ogr.CreateGeometryFromWkt(wkt)
                if geom:
                    geom.Transform(cad_common._CRS_TRANSFORM)
...
def write_geopackage(output_path, all_features, source_files,
    ...
    source_crs_label = source_crs_label or cad_common._SOURCE_CRS_LABEL
    target_crs_label = target_crs_label or cad_common._TARGET_CRS_LABEL
    ...
    for fc_name in layer_order:
        ...
        for fdata in feat_list:
            ...
            frame_span = 50.0 if cad_common._TARGET_IS_GEOGRAPHIC else 50.0 * METERS_PER_DEGREE
            origin_span = 10.0 if cad_common._TARGET_IS_GEOGRAPHIC else 10.0 * METERS_PER_DEGREE
            pts = pts_resolved
    ...
    for op_name, detail in [
        ...
        ("reprojection", transform_desc or (
            "identity (source CRS == target CRS)" if cad_common._CRS_TRANSFORM is None
...
def _load_project_config(path):
    """Load a JSON project configuration file.

    Returns a dict with the parsed config.  Missing keys are absent from the
    dict (the caller applies only what the file provides).
    """
    with open(path, 'r', encoding='utf-8') as fh:
        return json.load(fh)


def _apply_project_config(cfg, args):
    """Apply project-config overrides to module globals and CLI args.

    Keys present in *cfg* override their corresponding hardcoded default.
    Keys that are absent leave the default untouched (backward compatible).
    """
    global _PROJECT_CONFIG, REGION_BOUNDS_WGS84
    global LABEL_FAMILIES, LAYER_PATTERN_MAP
    global NEGATIVE_EVIDENCE_LAYERS, FRAGMENT_AGGREGATION_LAYERS
    global DEFAULT_FRAGMENT_CLUSTER_TOL_M, DEFAULT_ANNOTATION_LINK_TOL_M
    global DEFAULT_BOITE_FUSION_TOL_M

    _PROJECT_CONFIG = cfg

    # ── CRS overrides ────────────────────────────────────────────────────
    if 'source_crs' in cfg:
        args.source_crs = cfg['source_crs']
    if 'target_crs' in cfg:
        args.target_crs = cfg['target_crs']

    # ── Region bounds ────────────────────────────────────────────────────
    if 'region_bounds_wgs84' in cfg:
        REGION_BOUNDS_WGS84 = tuple(cfg['region_bounds_wgs84'])

    # ── Label families (full replacement) ────────────────────────────────
    if 'label_families' in cfg:
        LABEL_FAMILIES = cfg['label_families']

    # ── Layer pattern map (prepend: project patterns tried first) ────────
    if 'layer_pattern_map' in cfg:
        LAYER_PATTERN_MAP = cfg['layer_pattern_map'] + LAYER_PATTERN_MAP

    # ── Negative evidence layers (merged set) ────────────────────────────
    if 'negative_evidence_layers' in cfg:
        NEGATIVE_EVIDENCE_LAYERS = NEGATIVE_EVIDENCE_LAYERS | set(
            cfg['negative_evidence_layers'])

    # ── Fragment aggregation layers (full replacement) ───────────────────
    if 'fragment_aggregation_layers' in cfg:
        FRAGMENT_AGGREGATION_LAYERS = cfg['fragment_aggregation_layers']

    # ── Tolerances ───────────────────────────────────────────────────────
    tols = cfg.get('tolerances')
    if tols:
        if 'fragment_cluster_m' in tols:
            val = float(tols['fragment_cluster_m'])
            DEFAULT_FRAGMENT_CLUSTER_TOL_M = val
            args.fragment_cluster_tol = val
        if 'annotation_link_m' in tols:
            val = float(tols['annotation_link_m'])
            DEFAULT_ANNOTATION_LINK_TOL_M = val
            args.annotation_link_tol = val
        if 'boite_fusion_m' in tols:
            val = float(tols['boite_fusion_m'])
            DEFAULT_BOITE_FUSION_TOL_M = val
            args.boite_fusion_tol = val
        if 'snap_m' in tols:
            args.snap_tol = float(tols['snap_m'])
        if 'isolation_m' in tols:
            args.isolation_threshold = float(tols['isolation_m'])
        if 'chain_m' in tols:
            args.chain_tol = float(tols['chain_m'])
        if 'site_snap_max_m' in tols:
            args.site_snap_max_m = float(tols['site_snap_max_m'])
...
def main():
    ...
    parser.add_argument("--config", default=None,
                       help="Optional JSON project config (overrides defaults from "
                            "schema_config.py: label families, layer mapping, CRS, "
                            "tolerances, code prefixes, FDT values)")
    ...
    # Load project config and apply overrides (backward compatible: no-op
    # when --config is omitted)
    if args.config:
        cfg = _load_project_config(args.config)
        _apply_project_config(cfg, args)

    # Set up CRS pipeline: source → target (+ target → 4326 for region checks)
    cad_common.init_crs(args.source_crs, args.target_crs)

    # Compute transform description for metadata tables
    if cad_common._CRS_TRANSFORM is None:
...
def _assign_codes(all_features):
    ...
    code_prefix = (_PROJECT_CONFIG.get('code_prefix') if _PROJECT_CONFIG
                   else None) or {
                       "SITE": "PM", "BOITE": "PBO", "PTECH": "PT",
...
def _write_boite_attrib_values(gpkg_path, cache_path=None):
    ...
    """
    FAT_DEFAULT = (_PROJECT_CONFIG.get('fat_default') if _PROJECT_CONFIG
                   else None) or 16
    FDT_VALUE = (_PROJECT_CONFIG.get('fdt_value') if _PROJECT_CONFIG
                 else None) or {"FDT-01": 48, "FDT-02": 72}
    assigned = 0; total = 0
    ds = ogr.Open(gpkg_path, 1)
    if ds is None:
        return 0
